    ...
    <main>
        <table class="table table-bordered table-striped text-center" style="margin: 20px">
            ...
            <c:forEach var="ticket" items="${requestScope.ticketList}">
                <tr>
                    <td>${ticket.ID}</td>
                    <td>${ticket.route.train}</td>
                    <td>${ticket.route.departureCity}/${ticket.route.arrivalCity}</td>
                    <td>${ticket.route.startStation}/${ticket.route.arrivalStation}</td>
                    <td>${ticket.orderPrise} ₴</td>
                    <td>
                        <form action="controller" method="get">
                            <input type="hidden" name="action" value="purchasedTicket">
